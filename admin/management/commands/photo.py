...
class Command(NoArgsCommand):
    def handle_noargs(self, **options):
        ...
        self.handle_tag('instadaily')

        # supporting tags
        s = Subject.objects.get_current()

        if s is not None and s.tags is not None:
    ...
    def handle_tag(self, tag_name):
        ...
        url = 'https://api.instagram.com/v1/tags/%s/media/recent?access_token=627640.f59def8.9b0aa5e5317847579720140967ef1aa7' % tag_name
        if tag.min_tag_id > 0:
            url += '&min_tag_id=%d' % tag.min_tag_id
